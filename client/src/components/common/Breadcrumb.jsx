...
const Breadcrumb = ({ items = [], className }) => {
  return (
    <nav className={cn("flex items-center space-x-2 text-sm font-medium", className)} aria-label="Breadcrumb">
      
      {/* 1. Home Icon (Always Present) */}
      <Link 
        to="/" 
        ...
        return (
          <div key={index} className="flex items-center">
            ...
            {isLast ? (
              ...
              // Middle Links (Inactive - Gray to Teal)
              <Link 
                to={item.path}
